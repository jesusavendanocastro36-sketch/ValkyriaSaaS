...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const allowed = ['name','role','quote','result','sort_order','active'];
    const sets: string[] = [];
    const vals: unknown[] = [];
    ...
    const { rows } = await sitePool.query(
      `UPDATE testimonials SET ${sets.join(', ')} WHERE id = $${vals.length} RETURNING *`,
      vals
    );
    if (!rows[0]) return Response.json({ error: 'Testimonio no encontrado' }, { status: 404 });
...
export async function DELETE(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await params;
    await sitePool.query('UPDATE testimonials SET active = false WHERE id = $1', [id]);
